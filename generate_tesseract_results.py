...
def get_tesseract_results(image_path):
    ...
    ocr_result = pytesseract.image_to_data(image, config='--oem 1 --psm 11', output_type=Output.DICT)
    return ocr_result
...
if __name__ == '__main__':
    ...
    for image in tqdm(images, desc='Generating Tesseract Results'):
        ...
        with open(os.path.join(tesseract_results, image_name + '.json'),
                  'w') as f:
            json.dump(result, f)

        # imagesize = cv2.imread(str(image))
        # height, width, _ = imagesize.shape
        # with open(tesseract_results + "/" + image_name + ".json", 'r') as f:
        #     result = json.load(f)
        # candidate_data = extract_candidates_tesseract.get_candidates(result, height,
        #                                                              width)
        # file_path = config.CANDIDATE_DIR / (
        #         image.split('/')[-1].split('.')[0] + ".json")
        # with open(file_path, 'w') as outfile:
